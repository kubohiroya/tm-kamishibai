...
export type ConvertedAsset =
  | {kind: 'backdrop'; name: string; command: Dsl32Command}
  | {kind: 'sound'; name: string; sourceTarget: string; command: Dsl32Command}
  | {kind: 'costume'; name: string; sourceTarget: string; command: Dsl32Command}
  | {kind: 'recognitionModel'; file: string; loading?: 'eager' | 'lazy'; command: Dsl32Command}
  | {
...
export type ConversionReference = {
  kind: 'asset' | 'actor' | 'branch' | 'scene' | 'style';
  id: string;
  expectedKind?: 'backdrop' | 'costume' | 'sound' | 'recognitionModel';
  actor?: string;
  command: Dsl32Command;
};
...
function createDiagnostic(
  ...
  severity: 'error' | 'warning',
  message: string,
  command: Dsl32Command | null = null,
...
function compareDiagnostics(left: ConversionDiagnostic, right: ConversionDiagnostic) {
  return (
...
function decodeSource(input: string | Uint8Array) {
...
function canonicalizeSource(input: string | Uint8Array) {
...
function isSafePoseModelFile(file: string) {
...
function normalizePoseModels(input: unknown): Readonly<Record<string, PoseModelReplacement>> {
...
class Converter {
  ...
  loading: {backdrop?: string; costumes?: string[]} | null;
  recognition: {idleSound?: string; chargeSound?: string; sequence?: Record<string, number>} | null;
  currentScene: string | null;
  poseModels: Readonly<Record<string, PoseModelReplacement>>;
  ...
  validateIdentifier(id: string, label: string, command: Dsl32Command) {
  ...
  validateLiteralId(id: string, label: string, command: Dsl32Command) {
  ...
  parseVersion(command: Dsl32Command) {
  ...
  parseAsset(command: Dsl32Command) {
  ...
  parseAssetAddress(id: string, address: string, command: Dsl32Command): ConvertedAsset | null {
  ...
  parseActor(command: Dsl32Command) {
  ...
  parseVariable(command: Dsl32Command) {
  ...
  applyPoseRuntimeConfiguration() {
    ...
    if (this.variables.has('poseRecog')) {
      sequence.confidenceThreshold = confidenceThreshold as number;
    }
    if (this.variables.has('poseCharge')) {
      sequence.fullConfidenceHoldSeconds = 10 / (poseCharge as number);
    }
    if (this.variables.has('poseIdle')) sequence.idleChargePerSecond = 0;
    this.recognition.sequence = sequence;
  }

  parseLoadingBackdrop(command: Dsl32Command) {
  ...
  parseLoadingCostumes(command: Dsl32Command) {
  ...
  parsePoseRecognition(command: Dsl32Command) {
    const ids = splitList(command.value);
  ...
  parseSvgTextStyle(command: Dsl32Command) {
  ...
  parseLegacyText(command: Dsl32Command) {
  ...
  parseBranch(command: Dsl32Command) {
    const parts = command.value.split(':').map((part) => part.trim());
    if (parts.length !== 3) {
      this.error(
  ...
  parseScene(command: Dsl32Command) {
    const id = command.value.trim();
  ...
  parsePoseModel(command: Dsl32Command) {
    if (!this.currentScene) {
  ...
  validatePoseModelFile(file: string, command: Dsl32Command) {
  ...
  parseAction(command: Dsl32Command) {
    if (!this.currentScene) {
  ...
  parseGlobalAction(
    ...
    if (actionName === 'keyInputToChangeScene' || actionName === 'touchInputToChangeScene') {
      ...
      const mappedRoutes: Map<string, string> = new Map();
  ...
  parseActorAction(
    ...
    if (actionName === 'loop') {
      ...
      if (!durations.some((duration) => (duration as number) > 0)) {
        this.error(
          'K4-CONVERT-ACTION-ARGS',
      ...
      return {
        [key]: {
          steps: skins.map((skin, index) => ({
            skin,
            seconds: durations[index] as number,
          })),
        },
      };
    }
    ...
    if (actionName === 'pose') {
      ...
      this.scenesUsingPose.set(this.currentScene as string, command);
      return {[key]: {steps}};
    }
    this.error(
  ...
  parseNumber(
    ...
    label: string,
    command: Dsl32Command,
    limits: {minimum?: number; exclusiveMinimum?: number} = {},
  ) {
  ...
  addReference(
    kind: ConversionReference['kind'],
    id: string,
    command: Dsl32Command,
    details: Partial<ConversionReference> = {},
  ) {
    this.references.push({kind, id, command, ...details});
  }

  addCostumeUse(asset: string, actor: string) {
...
export function convertDsl32ToDsl4(
  input: string | Uint8Array,
  options: {sourceId?: string; poseModels?: Readonly<Record<string, PoseModelReplacement>>} = {},
): ConversionResult {
...
export function formatConversionDiagnostic(
