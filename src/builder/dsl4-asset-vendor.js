...
/** @param {string} relativePath @param {string} root @param {Map<string, Buffer>} files */
async function collectMirrorFiles(relativePath, root, files) {
...
/** @param {string} root @param {Map<string, Buffer>} expected */
async function verifyMirror(root, expected) {
...
/** @param {string} targetRoot @param {Map<string, Buffer>} files */
async function installMirrorAtomically(targetRoot, files) {
...
/** @param {string} outputPath @param {Buffer} contents @param {(candidate: string) => Promise<void>} validate */
async function writeJsonAtomically(outputPath, contents, validate) {
...
/** @param {Readonly<Record<string, any>>} config @param {Readonly<Record<string, string>>} embedded */
function createVendoredConfig(config, embedded) {
...
/** @param {Readonly<Record<string, any>>} lock @param {Readonly<Record<string, string>>} embedded */
function createVendoredLock(lock, embedded) {
...
/** @param {any} options */
export async function vendorDsl4AssetDistribution(options) {
  ...
  const files = new Map();
  const embedded = /** @type {Record<string, string>} */ ({});
  let totalBytes = 0;
  let fileCount = 0;
