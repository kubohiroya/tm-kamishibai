...
/** @param {string} target */
export function resolveDsl4PackagerEntrySourceSurface(target) {
...
function validatePackagerMetadata(metadata) {
...
/** @param {Readonly<Record<string, any>>} configuration */
function createBootstrapScript(configuration) {
...
function patchPackagerHtml(htmlBytes) {
...
/** @param {Readonly<Record<string, any>>} result @param {Readonly<{id: string, mode: string}>} surface */
function adaptPackagerResult(result, surface) {
 ...
 * @param {unknown} options.packager
 * @param {unknown} options.packagerPackage
 * @param {Readonly<Record<string, unknown>>} options.storyDocument
 * @param {unknown} options.descriptor
 * @param {{maxArchiveBytes: number, maxArchiveEntries: number, maxArchiveEntryBytes: number, maxArchiveExpandedBytes: number, maxAssetFiles: number, maxAssetFileBytes: number, maxAssetBytes: number, maxCompressionRatio: number, subtleCrypto?: {digest: Function}}} options.limits
 */
