...
function block(
  ...
  blockType: 'REPORTER' | 'BOOLEAN' = 'REPORTER',
  returns = 'scalar',
) {
  return {
    opcode,
...
export function resolveDsl4StructuredDataFeatureFlags(input: unknown = {}) {
...
export function createDsl4StructuredDataTurboWarpSurfaces(options: object = {}) {
  ...
  const standaloneHandlers = {
    ...
    createScope: (args) =>
      adapter.createScope(castString(Scratch, args.PARENT_SCOPE), castString(Scratch, args.LABEL)),
    ...
    iteratorCurrentKind: (args) => adapter.iteratorCurrentKind(castString(Scratch, args.ITERATOR)),
