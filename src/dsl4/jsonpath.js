...
function success(value) {
  return /** @type {Readonly<{ok: true, value: T}>} */ (Object.freeze({ok: true, value}));
}

/** @param {string} code @param {string} operation @param {string} message @returns {Readonly<{ok: false, error: Readonly<{code: string, operation: string, message: string}>}>} */
function failure(code, operation, message) {
  return /** @type {Readonly<{ok: false, error: Readonly<{code: string, operation: string, message: string}>}>} */ (
    deepFreeze({ok: false, error: {code, operation, message}})
  );
}
...
function normalizeLimits(inputLimits) {
  ...
  const defaults = /** @type {Readonly<Record<string, number>>} */ (dsl4JsonPathDefaultLimits);
  ...
  return Object.freeze(/** @type {typeof dsl4JsonPathDefaultLimits} */ (normalized));
}
...
/** @param {number} codePoint */
function isNameFirst(codePoint) {
  return (
...
/** @param {number} codePoint */
function isNameCharacter(codePoint) {
...
/** @param {string} query @param {number} index */
function queryCodePointAt(query, index) {
...
/** @param {any} state @param {string} [message] */
function syntax(state, message = 'The JSONPath query is not valid subset syntax') {
  throw new JsonPathFailure('SD-JSONPATH-SYNTAX', message);
}

/** @param {string} message */
function unsupported(message = 'The JSONPath query uses an unsupported feature') {
  throw new JsonPathFailure('SD-JSONPATH-UNSUPPORTED', message);
}

/** @param {any} state */
function unsupportedOrSyntax(state) {
...
/** @param {number} codePoint */
function hexDigitValue(codePoint) {
...
/** @param {any} state */
function readUnicodeEscape(state) {
  let value = 0;
...
/** @param {any} state */
function parseQuotedName(state) {
...
/** @param {any} state */
function parseNumericSelector(state) {
...
/** @param {any} state @param {any[]} selectors */
function appendSegment(state, selectors) {
...
/** @param {any} state */
function parseDotSegment(state) {
  state.index += 1;
...
/** @param {any} state */
function parseBracketSegment(state) {
  state.index += 1;
  skipWhitespace(state);
...
/** @param {string} query @param {any} limits */
function compileQuery(query, limits) {
  ...
  const state = /** @type {any} */ ({query, index: 1, limits, segments: [], astNodes: 1});
...
const rawJsonAdapter = Object.freeze({
  ...
  /** @param {unknown[]} node @param {number} index */
  arrayItem(node, index) {
    return node[index];
  },
});

/** @param {unknown} adapter @returns {any} */
...
/** @param {string} name */
function normalizedNameSegment(name) {
  let encoded = '';
...
/** @param {any} parent @param {string | number} key @param {any} limits @param {unknown} node */
function childResult(parent, key, limits, node) {
...
function normalizeArrayIndex(value, length) {
  return value >= 0 ? value : length + value;
}

/** @param {any} adapter @param {unknown} node */
function adapterKind(adapter, node) {
...
/** @param {any} adapter @param {unknown} node */
function* adapterObjectEntries(adapter, node) {
...
/** @param {any} adapter @param {unknown} node */
function adapterArrayLength(adapter, node) {
 ...
 * @param {Partial<typeof dsl4JsonPathDefaultLimits>} [options.limits]
 * @param {object} [options.adapter]
 */
export function createDsl4JsonPathEngine({limits: inputLimits, adapter = rawJsonAdapter} = {}) {
  ...
  function compile(query) {
    try {
      const program = compileQuery(/** @type {string} */ (query), limits);
  ...
  function validateProgram(program) {
    const candidate = /** @type {any} */ (program);
    if (
      ...
      candidate.segments.some(
        (/** @type {any} */ segment) => segment.selectors.length > limits.maxSelectorsPerSegment,
      )
    ) {
  ...
  /** @param {unknown} program @param {unknown} root @param {boolean} requireSingular */
  function evaluateProgram(program, root, requireSingular) {
    try {
      ...
      let visits = 0;
      /** @type {Array<Readonly<{node: unknown, path: readonly (string | number)[], normalizedPath: string}>>} */
      const results = [];

      /** @param {any} input @param {any} selector */
      ...
      /** @param {any} input @param {number} segmentIndex */
      function visit(input, segmentIndex) {
  ...
  /** @param {unknown} program @param {unknown} root */
  function evaluate(program, root) {
    return evaluateProgram(program, root, false);
  }

  /** @param {unknown} program @param {unknown} root */
  function evaluateSingular(program, root) {
    return evaluateProgram(program, root, true);
  }

  /** @param {unknown} root @param {unknown} query */
  function query(root, query) {
    const compiled = compile(query);
    return compiled.ok ? evaluate(compiled.value, root) : compiled;
  }

  /** @param {unknown} root @param {unknown} query */
  function querySingular(root, query) {
    const compiled = compile(query);
