...
export class Dsl4SourceGraphError extends Error {
  ...
  readonly range: Readonly<SourceRange> | null;
...
function fail(
  code: string,
  message: string,
  details?: ConstructorParameters<typeof Dsl4SourceGraphError>[2],
): never {
...
function pathText(value: unknown, name: string) {
...
function resolveSegments(baseSegments: readonly string[], reference: string, name: string) {
...
function sourceDirectorySegments(sourcePath: string) {
  const segments = resolveSegments([], sourcePath, 'sourcePath');
  segments.pop();
  return segments;
}

function requireSourceSuffix(sourcePath: string) {
...
export function resolveDsl4IncludePath(sourcePath: string, includePath: string) {
...
export function resolveDsl4SourceRelativeAssetPath(sourcePath: string, assetPath: string) {
  const source = requireSourceSuffix(resolveSegments([], sourcePath, 'sourcePath').join('/'));
  return resolveSegments(sourceDirectorySegments(source), assetPath, 'asset path').join('/');
}

function decodeSource(value: unknown, sourcePath: string, maxSourceBytes: number) {
  const byteLength =
    typeof value === 'string'
...
function parseSourceDocument(canonicalSource: string, sourcePath: string) {
 ...
 */
function inspectIncludes(canonicalSource: string, sourcePath: string) {
...
function inspectDeclarations(canonicalSource: string, sourcePath: string) {
  const {document, lineCounter} = parseSourceDocument(canonicalSource, sourcePath);
  const declarations: {
    namespace: string;
    name: string;
 ...
 */
export async function createDsl4SourceGraph(
  entryPath: string,
  {
    readSource,
    limits: inputLimits,
  ...
  while (queue.length > 0) {
    const sourcePath = queue.shift() as string;
  ...
  const visitState: Map<string, 'visiting' | 'visited'> = new Map();
  const stack: string[] = [];
  const order: string[] = [];
  ...
  const longestDepth: Map<string, number> = new Map();
  function depthFrom(sourcePath: string) {
    const known = longestDepth.get(sourcePath);
    if (known !== undefined) return known;
    const node = discovered.get(sourcePath);
    const depth = Math.max(
      0,
      ...(node?.includes ?? []).map((edge: {path: string}) => 1 + depthFrom(edge.path)),
    );
