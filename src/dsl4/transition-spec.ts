...
function seconds(value: unknown, path: string) {
  if (
    typeof value !== 'number' ||
...
export function normalizeDsl4VisualTransition(value: unknown, path: string = 'transition') {
...
export function normalizeDsl4AudioTransition(value: unknown, path: string = 'transition') {
  if (typeof value === 'number') {
    const duration = seconds(value, path);
    return duration === 0
      ? dsl4CutTransition
...
export function isDsl4CrossfadeTransition(value: unknown) {
  return isRecord(value) && value.effect === 'crossfade' && Number(value.seconds) > 0;
}

export function resolveDsl4TransitionDefaults(storyDocument: Readonly<Record<string, unknown>>) {
...
export function dsl4StoryUsesCrossfade(storyDocument: Readonly<Record<string, unknown>>) {
  return dsl4FirstCrossfadeStoryPath(storyDocument) !== null;
}

export function dsl4FirstCrossfadeStoryPath(storyDocument: Readonly<Record<string, unknown>>) {
  const presentation = isRecord(storyDocument.presentation) ? storyDocument.presentation : {};
  const transitions = isRecord(presentation.transitions) ? presentation.transitions : {};
