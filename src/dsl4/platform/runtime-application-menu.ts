...
export function createDsl4RuntimeApplicationMenu(options: {
  ...
  const root = rootCandidate as Record<string, any>;
  ...
  const definitionList: Array<{
    action: 'open' | 'reload' | 'build' | 'about' | 'language';
    left: string;
    top: string;
  ...
  for (const definition of definitions) {
    ...
    const onClick = () => {
      ...
      if (definition.callback) invoke(definition.callback as () => unknown);
    };
  ...
  const status = statusCandidate as Record<string, any>;
