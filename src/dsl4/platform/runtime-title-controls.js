...
export function createDsl4RuntimeTitleControls(options) {
  ...
  /** @param {Record<string, any>} event @param {Function} operation */
  ...
  const onWebsiteClick = (event) => invoke(event, options.onWebsite);
  /** @param {Record<string, any>} event */
  const onCloseClick = (event) => invoke(event, options.onClose);
