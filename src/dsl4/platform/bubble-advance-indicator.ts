...
function indicatorError(message: string) {
...
export function createDsl4BubbleAdvanceIndicatorPresenter(options: unknown) {
  ...
  const activeOperations = new Set<Readonly<{stop: () => void}>>();
  let disposed = false;

  function markChanged(target: Record<string, any>, skin: Record<string, any>) {
  ...
  function installRenderer(target: Record<string, any>, active: Record<string, any>) {
    ...
    const reflow = function (this: Record<string, any>) {
    ...
    const render = function (this: Record<string, any>, scale: number) {
  ...
  return Object.freeze({
    create(targetInput: unknown, indicatorInput: unknown) {
      ...
      const target = targetInput as Record<string, any>;
      const images = indicatorInput.frames.map((assetId) => {
        ...
        const image = createImage() as Record<string, any>;
