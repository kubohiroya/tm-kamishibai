...
function storedAssetDescriptors(project) {
 ...
 * @param {unknown} project
 */
export function inspectDsl4PackagedBinaryRuntime(project) {
...
export function resolveDsl4PackagerSessionPolicy(surface) {
 ...
 * @param {Readonly<{maxAssetFiles: number, maxAssetBytes: number}>} defaults
 */
function resolveAssetLimits(descriptor, defaults) {
 ...
 * @param {Record<PropertyKey, any>} [options.globalObject]
...
export async function createDsl4PackagedBinaryRuntimeBridge(options) {
  ...
  const loaded = /** @type {Readonly<{storyDocument: Readonly<Record<string, unknown>>}>} */ (
