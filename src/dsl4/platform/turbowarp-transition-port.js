...
function validateScheduler(value) {
  ...
  return /** @type {{setTimeout: (callback: () => void, milliseconds: number) => ReturnType<typeof setTimeout>, clearTimeout: (handle: ReturnType<typeof setTimeout>) => void}} */ (
    value
  );
}
...
function validatePayload(value) {
  ...
  return /** @type {{effect: keyof typeof transitionEndpoints, seconds: number}} */ (
    /** @type {unknown} */ (value)
  );
}
...
export function createDsl4TurboWarpTransitionPort(options) {
  ...
  const stage =
    /** @type {Record<string, unknown> & {setEffect: (effect: string, value: number) => void}} */ (
      stageCandidate
    );
