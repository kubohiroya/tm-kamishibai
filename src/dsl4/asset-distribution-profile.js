 ...
 * @param {string} code
 */
function strictKeys(value, allowed, required, name, code) {
...
/** @param {unknown} value @param {string} name @param {string} code */
function nonEmptyString(value, name, code) {
...
/** @param {unknown} value @param {string} name @param {string} code */
function delivery(value, name, code) {
  if (!deliveryPolicies.has(String(value))) {
    fail(code, `${name} must be embedded or remote`);
  }
  return /** @type {'embedded' | 'remote'} */ (value);
}

/** @param {unknown} value @param {string} name @param {string} code */
function integrity(value, name, code) {
...
/** @param {unknown} value @param {string} name @param {string} code */
function contentType(value, name, code) {
...
/** @param {unknown} value @param {string} name @param {string} code @param {number} minimum */
function safeSize(value, name, code, minimum) {
...
/** @param {unknown} value @param {string} name @param {string} code */
function safeRelativePath(value, name, code) {
...
/** @param {unknown} value @param {string} name @param {string} code */
function httpsUrl(value, name, code) {
...
/** @template T @param {Record<string, unknown>} value @param {(child: unknown, key: string) => T} map */
function sortedRecord(value, map) {
...
/** @param {unknown} input @param {string} name @param {string} code */
function embeddedProvider(input, name, code) {
...
function remoteConfigProvider(input, name) {
...
/** @param {unknown} input @param {string} name */
function configProviderSet(input, name) {
...
/** @param {unknown} input @param {string} name */
function distributionProfile(input, name) {
  ...
  const assets = Object.hasOwn(profile, 'assets')
    ...
    : {};
  return {
    network: /** @type {'allowed' | 'forbidden'} */ (profile.network),
 ...
 * @param {unknown} input
 */
export function validateDsl4AssetDistributionConfig(input) {
...
/** @param {unknown} input @param {string} name */
function remoteLockProvider(input, name) {
...
/** @param {unknown} input @param {string} name */
function lockProviderSet(input, name) {
 ...
 * @param {unknown} input
 */
export function validateDsl4AssetDistributionLock(input) {
...
function sameEmbeddedProvider(left, right) {
  return left.file === right.file && left.name === right.name;
}

/**
 * @param {string} assetId
 * @param {Readonly<Record<string, any>>} story
 * @param {Readonly<Record<string, any>> | undefined} configured
 */
function declaredProviders(assetId, story, configured) {
  const result = {...story};
...
/**
 * @param {string} assetId
 * @param {Readonly<Record<string, any>>} declared
 * @param {Readonly<Record<string, any>>} locked
 */
function bindProviders(assetId, declared, locked) {
...
/** @param {Readonly<Record<string, any>>} asset @param {'embedded' | 'remote'} selected @param {Readonly<Record<string, any>>} provider */
function resolvedStoryAsset(asset, selected, provider) {
  const result = /** @type {Record<string, any>} */ ({...asset, delivery: selected});
  delete result.file;
  delete result.name;
 ...
 * @param {unknown} inputConfig
 * @param {unknown} inputLock
...
export function resolveDsl4AssetDistributionProfile(
  ...
  const profile = /** @type {Readonly<Record<string, any>> | undefined} */ (
  ...
  const resolvedAssets = Object.fromEntries(
    storyIds.map((assetId) => {
      ...
      const lockAssetEntry = /** @type {Readonly<Record<string, any>>} */ (lock.assets[assetId]);
      ...
      const selected = /** @type {'embedded' | 'remote'} */ (
        profile.assets?.[assetId] ??
...
/** @param {unknown} input @param {string} name */
function resolutionProvider(input, name) {
 ...
 *
 * @param {Readonly<Record<string, unknown>>} sourceStoryDocument
 * @param {unknown} input
 */
export function validateDsl4AssetDistributionResolution(sourceStoryDocument, input) {
  ...
  const sourceWithoutAssets = /** @type {Record<string, unknown>} */ (
    structuredClone(sourceStoryDocument)
  );
  const resolvedWithoutAssets = /** @type {Record<string, unknown>} */ (
    structuredClone(resolvedStory)
  );
  ...
  const normalizedAssets = resolution.assets.map((candidate, index) => {
    ...
    const normalized = {
      ...
      delivery: /** @type {'embedded'|'remote'} */ (asset.delivery),
      contentIntegrity: integrity(
        asset.contentIntegrity,
