 ...
 * @param {{poseState?: unknown, version?: unknown, disposed?: boolean}} [options]
 */
export function createDsl4RuntimeVariableSnapshot(
  runtimeState,
  {poseState, version, disposed = false} = {},
) {
...
/** @param {string} key */
function syntheticVariableName(key) {
 ...
 */
export function lowerDsl4RuntimeExpression(expression) {
 ...
 * @param {unknown} input
 */
export function createDsl4RuntimeStateExpressionComposition(input) {
  if (!isRecord(input) || !isRecord(input.composition)) {
    throw new TypeError('Runtime state expression adapter requires a composition');
  }
  const composition = /** @type {Record<string, Function>} */ (input.composition);
  if (
  ...
  return Object.freeze({
    ...
    /**
     * @param {string} expression
     * @param {Readonly<Record<string, string | number | boolean>>} variables
     * @param {unknown} runtimeSnapshot
     */
    evaluateCondition(expression, variables, runtimeSnapshot = {}) {
