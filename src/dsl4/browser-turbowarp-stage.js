...
/** @param {unknown} input @param {number} maximum */
function projectBytes(input, maximum) {
...
/** @param {Record<string, any>} canvas @param {Record<string, any>} event */
function pointerData(canvas, event) {
...
export function createDsl4BrowserTurboWarpStage(options) {
  ...
  const canvas = /** @type {Record<string, any>} */ (canvasCandidate);
  canvas.width = width;
  ...
  let vm = null;
  /** @type {ReturnType<typeof createTurboWarpRuntimeHost> | null} */
  ...
  function snapshot() {
    ...
    return deepFreeze({
      ...
      hasStage: targets.some(
        /** @param {unknown} target */ (target) => isRecord(target) && target.isStage === true,
      ),
  ...
  function showApplicationMenu(locale) {
    ...
    const stage = /** @type {Record<string, any>} */ (runtimeHost.getStageTarget());
