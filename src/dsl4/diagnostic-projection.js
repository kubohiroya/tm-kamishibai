...
/** @param {string} canonicalSource @param {number} line */
function sourceLine(canonicalSource, line) {
  if (line < 1) return '';
  let currentLine = 1;
  let start = 0;
...
export function redactDsl4DiagnosticTelemetry(diagnostic) {
  ...
  const record = /** @type {Readonly<Record<string, unknown>>} */ (canonical);
  return deepFreeze(
    Object.fromEntries(
      telemetryFields
 ...
 * @param {string} options.canonicalSource
 * @param {string} options.displayName
 * @param {Partial<typeof dsl4DiagnosticProjectionDefaults>} [options.limits]
 */
export function createDsl4DiagnosticUiProjection(
  diagnostics,
  {canonicalSource, displayName, limits = {}},
) {
...
/** @param {Readonly<Record<string, any>>} projectedDiagnostic */
export function formatDsl4DiagnosticClipboard(projectedDiagnostic) {
...
/** @param {Readonly<Record<string, any>>} projection */
export function renderDsl4DiagnosticFallbackSvg(projection) {
...
/** @param {unknown} diagnostics @param {{maxDiagnostics?: number, maxRelatedLocations?: number}} [limits] */
export function serializeDsl4DiagnosticExport(diagnostics, limits = {}) {
  const normalized = normalizeDsl4DiagnosticSequence(diagnostics, {
