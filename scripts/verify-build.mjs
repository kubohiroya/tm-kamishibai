...
const {PDFDict, PDFDocument, PDFName, PDFNumber} = vivliostyleRequire('pdf-lib');
...
async function verifySiteAppBars() {
  ...
  for (const htmlFile of htmlFiles) {
    ...
    assert(
      html.split(stylesheet).length - 1 === 1,
...
async function verifySiteIndex() {
  ...
  assert(
    images.includes('images/image01.png'),
  ...
  assert(
    allLinks.includes(urashimaWebUrl),
  ...
  assert(
    allLinks.includes(sampleSiteUrl),
...
async function verifyDownloads(titleBuildMetadata) {
  ...
  const [sourceEntries, publishedEntries, publishedArchive, archiveStat, packageJsonSource] =
    await Promise.all([
  ...
  assert(
    Array.isArray(expectedBuild.source.project.targets),
...
async function verifyGeneralDocuments(grade) {
  ...
  let rubyCount = 0;
  const generatedHeadingIds = new Map(
    await Promise.all(
  ...
  assert(
    generalTheme.includes('counter-increment: general-toc-chapter;') &&
      generalTheme.includes('counter-increment: general-toc-section;'),
    'The general theme does not generate table-of-contents numbers.',
  );
  assert(
    generalTheme.includes('target-counter(attr(href), general-chapter)') &&
  ...
  assert(
    /#toc > ol > li\[data-section-level="1"\]:only-child > a\s*\{\s*display: none;/u.test(
      generalTheme,
    ) &&
      /#toc > ol > li\[data-section-level="1"\]:only-child > ol\s*\{\s*padding-left: 0;/u.test(
        generalTheme,
      ),
  ...
  for (const generalDocument of generalDocumentConfig.documents) {
    ...
    const standaloneTocLinks = await verifyLocalReferences(standaloneTocPath, 'a', 'href');
    ...
    for (const match of source.matchAll(/\]\((?!https?:)([^)\s]+\.md)#([^)]+)\)/gu)) {
      ...
      assert(
        generatedHeadingIds.get(targetFilename)?.has(decodeURIComponent(fragment)),
    ...
    for (const referenceType of ['chapter', 'section']) {
      const sourceReferenceCount = (
        source.match(new RegExp(`\\]\\([^\\n)]+\\)\\{data-ref="${referenceType}"\\}`, 'gu')) ?? []
      ).length;
      const generatedReferenceCount = attributeValues(standaloneArticle, 'a', 'data-ref').filter(
        (value) => value === referenceType,
      ).length;
      assert(
        sourceReferenceCount === generatedReferenceCount,
    ...
    if (generalDocument.sourceFilename === '02-executive-summary-kids.md') {
      assert(
        !source.includes('忍者') &&
          !source.includes('銅像') &&
          !standaloneArticle.includes('忍者') &&
          !standaloneArticle.includes('銅像'),
    ...
    assert(
      standaloneManifest.readingOrder[0].rel === 'contents',
    ...
    const tocHeadingIds = standaloneTocLinks.flatMap((reference) => {
      const [relativePath, encodedFragment] = reference.split('#');
      return relativePath === generalDocumentConfig.standaloneArticleHtmlFilename && encodedFragment
        ? [decodeURIComponent(encodedFragment)]
        : [];
    });
    assert(
      JSON.stringify(tocHeadingIds) === JSON.stringify(bodyHeadingIds),
    ...
    assert(
      documentBuildInfo?.rubyApplied === shouldAddFurigana,
      `${htmlFilename} has inconsistent rubygana build metadata.`,
    );
    assert(
      documentBuildInfo?.webPublicationDirectory === basename,
      `${htmlFilename} build metadata does not record its standalone Web Publication.`,
    );
    assert(
      documentBuildInfo?.generatedTableOfContents?.htmlFilename ===
        generalDocumentConfig.standaloneTocHtmlFilename &&
    ...
    if (shouldAddFurigana) {
      assert(
        standaloneArticle.includes(`data-rubygana-grade="${grade}"`),
        `${htmlFilename} does not record the configured rubygana grade.`,
      );
      assert(documentRubyCount >= 20, `${htmlFilename} was not processed by rubygana.`);
      assert(
        documentBuildInfo.learnedThroughGrade === grade,
        `${htmlFilename} does not record its learned-through grade.`,
      );
      assert(
        codeBlocks.every((block) => !block.includes('<ruby')),
      ...
      assert(
        documentRubyCount === 0 && !standaloneArticle.includes('data-rubygana-grade='),
        `${htmlFilename} was unexpectedly processed by rubygana.`,
      );
    }
    assert(
      readingOrder.includes(htmlFilename),
      `${htmlFilename} is missing from the general publication reading order.`,
    );
    assert(
      docsIndexLinks.includes(`${generalDocumentConfig.outputDirectory}/${basename}/`),
      `${basename}/ is missing from the documentation entrance page.`,
    );
    assert(
      docsIndexLinks.includes(`${generalDocumentConfig.outputDirectory}/${pdfFilename}`),
  ...
  assert(
    generalTocLinks.length >= generalDocumentConfig.documents.length,
    'General publication TOC does not link to every document.',
  );
  assert(
    rubyDocumentCount === 1 && rubyCount > 0,
    `Expected one furigana-enabled general document, found ${rubyDocumentCount}.`,
  );
  assert(
    docsIndexLinks.includes(`${documentConfig.outputDirectory}/${documentConfig.pdfFilename}`),
...
async function verifyStaffDocument() {
  ...
  assert(
    source.startsWith(`# ${staffDocumentConfig.title}\n`),
    'Staff Markdown does not start with the configured title.',
  );
  assert(
    /^# 1\. 体験会運営用資料$/mu.test(source),
    'Staff Markdown does not contain the reorganized operations section.',
  );
  assert(
    !/^## 2\. アプリ$/mu.test(source) && !/^## 3\. (?:関連)?ライブラリ(?:など)?$/mu.test(source),
    'Staff Markdown still contains developer documentation.',
  );
  assert(!/^#{1,3} [ABC]\./mu.test(source), 'Staff Markdown still uses appendix-style headings.');
  assert(
    html.includes(staffDocumentConfig.title),
    'Staff HTML does not contain its configured title.',
  );
  assert(
    !html.includes('<ruby') && !html.includes('data-rubygana-grade='),
    'Staff HTML was unexpectedly processed by rubygana.',
  );
  assert(
    contentImages.length === 1 && contentImages[0] === 'images/image03.jpg',
    'Staff HTML does not contain the expected local venue map.',
  );
  assert(
    JSON.stringify(readingOrder) === JSON.stringify([staffDocumentConfig.htmlFilename]),
    `Unexpected staff publication reading order: ${readingOrder.join(', ')}`,
  );
  assert(
    buildInfo.publicationKind === 'workshop-staff-documentation' && buildInfo.rubyApplied === false,
    'Staff build metadata does not identify a non-ruby staff publication.',
  );
  assert(
    docsIndexLinks.includes(`${staffDocumentConfig.outputDirectory}/`),
    'Staff HTML is missing from the documentation entrance page.',
  );
  assert(
    docsIndexLinks.includes(
  ...
  assert(publishedPdf.equals(outputPdf), 'Staff PDF differs between dist/docs and output/pdf.');
...
export async function verifyBuild({titleBuildMetadata} = {}) {
  ...
  const tocLinks = (await verifyLocalReferences(tocPath, 'a', 'href')).filter(
    (reference) => reference !== '#main-content',
  );
