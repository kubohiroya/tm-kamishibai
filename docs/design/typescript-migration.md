...
- `src/dsl4/platform/generated/posenet-bundle-assets.js` — the embedded PoseNet model data,
  regenerated by `pnpm posenet-bundle-assets:generate`.

Both sit under a `generated/` directory so that `.gitattributes`, `.prettierignore` and the ESLint
ignore list can name them with one `**/generated/**` glob rather than listing each artifact
separately. `.gitattributes` marks them `linguist-generated=true -diff`, because a regeneration
rewrites 19,705 lines of minified bundle or a single 6.5 MB base64 literal and rendering that as a
diff helps nobody. `tsconfig.json` excludes only the playback runtime bundle: it is read through
`new URL(..., import.meta.url)` and never imported, so excluding it takes it out of the program. The
PoseNet module is imported by `src/builder/dsl4-local-preview-browser-entry.ts`, and an imported file
enters the program whatever `exclude` says -- which is why its generator emits the JSDoc that types
`embeddedPoseNetFiles` for that consumer.
