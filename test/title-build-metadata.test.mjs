...
test('embeds package version and build date into the Title SVG without changing app source', async () => {
  const [packageJson, projectSourceBefore, sourceManifestBefore, assetFilenamesBefore] =
    await Promise.all([
  ...
  assert(titleSvg.includes('text-anchor="middle"'));
  ...
  assert.equal(archive['79b33d9cca337f9e9c01aaf759281b03.svg'], undefined);
