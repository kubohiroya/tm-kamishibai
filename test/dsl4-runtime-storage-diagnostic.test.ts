...
test('projects bounded localized session backing recovery guidance without payload data', () => {
  const warning = createDsl4SessionBackingWarningDiagnostic(
    {
      ...
      causeCode: 'KVS_SESSION_BINARY_QUOTA_EXCEEDED',
      bytes: 'do-not-render',
  ...
  assert.equal(
    warning.code,
    'KVS_SESSION_BINARY_DIRECT_FALLBACK (KVS_SESSION_BINARY_QUOTA_EXCEEDED)',
  );
  assert.match(warning.message, /直接読み込んで続行/u);
  assert.doesNotMatch(JSON.stringify(warning), /do-not-render|private/u);

  const cases = [
    ['KVS_SESSION_BINARY_QUOTA_EXCEEDED', /site data|direct ZIP\/Electron/iu],
    ['KVS_SESSION_BINARY_INDEXEDDB_BLOCKED', /Close other copies/iu],
    ['KVS_SESSION_BINARY_CORRUPT', /missing or damaged/iu],
    ['KVS_SESSION_BINARY_SOURCE_INTEGRITY_MISMATCH', /Rebuild the SB3/iu],
    ['KVS_SESSION_BINARY_INDEXEDDB_UNAVAILABLE', /browser storage settings/iu],
  ] as const;
  ...
  const nested = createDsl4SessionBackingFatalDiagnostic(
    {
      code: 'KVS_SESSION_BINARY_SOURCE_READ_FAILED',
      cause: {code: 'K4-ASSET-ENTRY-INTEGRITY-001'},
    },
    'en',
  );
  assert.equal(nested.code, 'KVS_SESSION_BINARY_SOURCE_READ_FAILED (K4-ASSET-ENTRY-INTEGRITY-001)');
