...
test('generates Standard 4.0 transiently and treats legacy 3.2 as a release asset', () => {
  assert.deepEqual(contract.bundleBoundaries.standard4, {
    ...
    provenance: [
      ...
      'release-metadata/4.0.0-rc.7.json',
      'package.json',
      'pnpm-lock.yaml',
      'LICENSES.md',
    ],
...
test('pins a deterministic release, publication, and rollback sequence', async () => {
  ...
  assert.match(metadata.artifact.url, /\/releases\/download\/v4\.0\.0-rc\.7\//u);
  ...
  assert.deepEqual(contract.releaseLifecycle, {
    metadata: 'release-metadata/4.0.0-rc.7.json',
    ...
    nextCandidateVersion: '4.0.0-rc.8',
    npmDistTag: 'next',
    gitTag: 'v4.0.0-rc.7',
