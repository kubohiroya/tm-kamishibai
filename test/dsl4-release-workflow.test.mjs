...
test('updates atomically, remains idempotent, and preserves the candidate on failure', async () => {
  ...
  try {
    ...
    await writeFile(
      path.join(root, 'release-sources/4.0.0-rc.6/release.json'),
      '{"invalid":true}\n',
    );
    await assert.rejects(updateDsl4Release(options), /metadata format is invalid/u);
    assert.equal(
      await readFile(path.join(root, 'release-sources/4.0.0-rc.6/app/project.source.json'), 'utf8'),
      '{"targets":[]}\n',
    );
  } finally {
...
test('checks candidates against the generator and frozen releases against their snapshot', async () => {
  ...
  try {
    ...
    const published = await recordDsl4Publication(
      {
        npmUrl: 'https://www.npmjs.com/package/example/v/4.0.0-rc.6',
        githubReleaseUrl: 'https://github.com/example/project/releases/tag/v4.0.0-rc.6',
